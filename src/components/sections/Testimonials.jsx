...
export function Testimonials() {
  ...
  return (
    <section className="relative py-20 md:py-32 px-4 md:px-6 lg:px-8 bg-surface/20">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-12 md:mt-16 max-w-4xl mx-auto relative">
          <Quote className="absolute -top-4 md:-top-6 -left-1 md:-left-2 size-12 md:size-16 text-glow/10" />
          ...
          <div className="relative min-h-[240px] md:min-h-[260px]">
            <AnimatePresence mode="wait">
              ...
              >
                <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-light text-foreground leading-snug tracking-tight">
                  "{t.quote}"
                </p>
                <div className="mt-8 md:mt-10 flex flex-col items-center gap-3 md:gap-4">
                  <div className="size-10 md:size-12 rounded-full bg-surface-elevated border border-border flex items-center justify-center text-glow font-mono text-xs md:text-sm">
                    {t.name
                      .split(" ")
                      .map((n) => n[0])
                      .join("")}
                  ...
                  <div>
                    <div className="font-medium text-sm md:text-base text-foreground">{t.name}</div>
                    <div className="font-mono text-[9px] md:text-[10px] uppercase tracking-widest text-muted-foreground mt-1">
                      {t.role}
                    </div>
                  </div>
                </div>
          ...
          <div className="mt-8 md:mt-12 flex items-center justify-center gap-4 md:gap-6">
            <button
              onClick={prev}
              className="size-8 md:size-10 flex items-center justify-center border border-border text-muted-foreground hover:text-glow hover:border-glow transition-all flex-shrink-0"
              aria-label="Previous"
            >
              ...
              {testimonials.map((_, idx) => (
                <button
                  ...
                  className={`h-px transition-all ${
                    idx === i ? "w-8 md:w-10 bg-glow" : "w-3 md:w-5 bg-border"
                  }`}
